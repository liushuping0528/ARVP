...
from models.prompters import PadPrompter
import torch
class DataManager(object):
    def __init__(self, dataset_name, shuffle, seed, init_cls, increment, pad_prompter_args=True):
    ...
    def get_dataset(
        ...
        if ret_data:
            return data, targets, DummyDataset(data, targets, trsf, self.use_path,self.pad_prompter_args)
        else:
            return DummyDataset(data, targets, trsf, self.use_path,self.pad_prompter_args)
    ...
    def get_finetune_dataset(self,known_classes,total_classes,source,mode,appendent,type="ratio"):
        ...
        return DummyDataset(val_data, val_targets, trsf, self.use_path,self.pad_prompter_args)
    ...
    def get_dataset_with_split(
        ...
        return DummyDataset(
            train_data, train_targets, trsf, self.use_path, self.pad_prompter_args
        ), DummyDataset(val_data, val_targets, trsf, self.use_path, self.pad_prompter_args)
...
class DummyDataset(Dataset):
    def __init__(self, images, labels, trsf, use_path=False, pad_prompter_args=None):
        assert len(images) == len(labels), "Data size error!"
        # device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
        # prompter = PadPrompter().to(device) 
